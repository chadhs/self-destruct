...
** Prerequisites
   ...
   You will need [[https://github.com/technomancy/leiningen][Leiningen]] 2.11.0 or above installed (2.12.x recommended).
...
** Database Migrations
   ...
   - migrations are applied via ~lein migrate~ (local; runs the app ~--migrate~ CLI) or ~java -jar self-destruct.jar --migrate~ (production / Heroku release phase).
   - for local development set ~DATABASE_URL~ to a JDBC URL Postgres will accept over TCP.
   - ~SESSION_COOKIE_KEY~ should be a 16-character/byte secret (Ring cookie session store).
